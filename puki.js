    ...
    try {
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          m.plugin = name
          if (name === 'menu.js' && (m.sender === '6283863727744@s.whatsapp.net' || m.sender === '6283863727744@s.whatsapp.net')) m.reply('Hai Dev!')
                       ...
                       let pp = 'https://telegra.ph/file/2e77ffa63ce0b7f8c5163.jpg'
                        try {
                            ...
                            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Yah,si Beban Masuk Grup @user').replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
                                ...
                                this.sendButtonImg(id, pp, text, wm, "WELCOMEK", ".intro", null)
                                }
                    }
                }
                break
...
global.dfail = (type, m, conn) => {
  ...
  global.dfails = (jid, buffer, content, footer, button1, row1, button2, row2, quoted) => {
          ...
          `, '©JarotOFC', 'Terima', '.terima', 'Tidak', '.tidak', m)
